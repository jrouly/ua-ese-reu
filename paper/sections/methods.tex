...
\end{figure*}
%
\Fig{fig:featuremodel} is a feature model describing the features we
considered for the visual \acp{ide}. They are defined in
\Sect{subsec:featuredefinition}.
...
%That is, the field or fields for which the interface is primarily used, if
%any.
...
workspace, \emph{undo} the user's most recent action.
 %to place an existing syntactic element from the workspace into paste
 %buffer,
...
\Fig{fig:clutterexample}) do not offer options for interface organization
...
\Sect{subsec:ideevaluation} includes details on the proper evaluation of
...
essential use case $E$ as depicted in \Eq{eq:eefficiency}. A concrete use
...
be measured through simple use case analysis. \Sect{subsec:ideevaluation}
...
steps in the essential use case, as depicted in \Eq{eq:iefficiency}.
%
...
This nominal variable can take the values: \texttt{None} when no keyboard
...
\Fig{fig:clutterexample} as an example image provided on \ac{mt}. These
